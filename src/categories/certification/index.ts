...
export class CertificationApi {
  ...
  /**
   * Получить информацию о товарах, привязанных к сертификату
   ...
   * @example
   * ```typescript
   * const products = await certificationApi.getCertificateProducts({
   *   certificate_id: 12345,
   *   page: 1,
   *   page_size: 50
   * });
   ...
   * ```
   */
  async getCertificateProducts(
    request: CertificateInfoFromListRequest,
    options?: RequestOptions
  ): Promise<CertificateInfoFromListResponse> {
    return this.httpClient.request<CertificateInfoFromListRequest, CertificateInfoFromListResponse>(
      'POST',
   ...
   * @example
   * ```typescript
   * const accordanceTypes = await certificationApi.getCertificateAccordanceTypes();
   * 
   ...
   * ```
   */
  async getCertificateAccordanceTypes(
    options?: RequestOptions
