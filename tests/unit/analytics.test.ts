...
import type { AnalyticsTurnoverStocksRequest, AnalyticsStockOnWarehouseRequest } from "../../src/types/requests/analytics.js";
import type { AnalyticsTurnoverStocksResponse, AnalyticsStockOnWarehouseResponse } from "../../src/types/responses/analytics.js";
...
describe("AnalyticsApi", () => {
  ...
  describe("getStocksTurnover", () => {
    it("should call correct endpoint with request data", async () => {
      ...
      expect(mockHttpClient.request).toHaveBeenCalledWith("POST", "/v1/analytics/turnover/stocks", request, undefined);
  ...
  describe("getStockOnWarehouses", () => {
    it("should call correct endpoint with required limit parameter", async () => {
      ...
      expect(mockHttpClient.request).toHaveBeenCalledWith("POST", "/v2/analytics/stock_on_warehouses", request, undefined);
      expect(result).toEqual(mockResponse);
    });
    ...
    it("should work with different warehouse types", async () => {
      ...
      expect(mockHttpClient.request).toHaveBeenCalledWith("POST", "/v2/analytics/stock_on_warehouses", fulfillmentRequest, undefined);
      ...
      expect(mockHttpClient.request).toHaveBeenCalledWith("POST", "/v2/analytics/stock_on_warehouses", crossdockRequest, undefined);
    });
  ...
  describe("error handling", () => {
    it("should propagate HTTP client errors", async () => {
      ...
      await expect(analyticsApi.getStocksTurnover({})).rejects.toThrow("Network error");
    });
  });
