...
describe("OzonSellerApiClient Integration", () => {
  ...
  describe("connection testing", () => {
    it("should test connection successfully", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith("https://api-seller.ozon.ru/v1/seller/info", expect.any(Object));
    });
  ...
  describe("raw requests", () => {
    ...
    it("should make raw GET request", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith("https://api-seller.ozon.ru/test/endpoint", expect.objectContaining({ method: "GET" }));
    });
    ...
    it("should throw error for unsupported HTTP method", async () => {
      await expect(client.rawRequest("PATCH" as any, "/test/endpoint")).rejects.toThrow(ConfigurationError);
    });
  });
});
